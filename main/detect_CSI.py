...
object_class = ['pingpong_new']
Object_detector = OBJ_DETECTION("best.pt", object_class)
ip = '10.0.0.10'
port = 65432
...
def run(
    ...
    source = str(source)
    if source == 3:
        source = nano.Camera(flip= 2, width = 640, height= 480, fps=30)
        print("CSI Camera ready", source.isReady())

        while(source.isReady()):
            frame = source.read()
            objs = Object_detector(frame)
            for obj in objs:
                label = obj['label']
                score = obj['score']
                [(xmin, ymin), (xmax,ymax)] = obj['bbox']
                frame = cv2.rectangle(frame, (xmin,ymin), (xmax,ymax), 2)
                frame = cv2.putText(frame, f'{label} ({str(score)})', (xmin,ymin), cv2.FONT_HERSHEY_SIMPLEX , 0.75, 1, cv2.LINE_AA)
                ...
                for *xyxy, conf, cls in reversed(det):
                    ...
                    if save_img or save_crop or view_img:  # Add bbox to image
                        ...
                        if(ballMidX>x3[0] and ballMidX < x5[0]):
                             sent_data = "in the middle"
                             print(sent_data)
                             #sock1.sendall(sent_data.encode('utf-8'))
                             
                        elif(ballMidX<x3[0]):
                             sent_data = "on the left"
                             print(sent_data)
                             #sock1.sendall(sent_data.encode('utf-8'))
                             
                        else:
                             sent_data = "on the right"
                             print(sent_data)
                             #sock1.sendall(sent_data.encode('utf-8'))
